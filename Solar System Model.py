 # -*- coding: utf-8 -*-
"""
Created on Wed Sep 28 15:41:20 2022
...
daysec      = 24.0*60*60   # Seconds within 1 day
#########################################
#
...
commet_v = 7000                # Unkown Comet Velocity
########################################
#
# Eccentricity##########################
e_me = 0.20563069
...
gravconst_mer = G*Mmer*Ms # Mercury
gravconst_v   = G*Mv*Ms   # Venus
gravconst_e   = G*Me*Ms   # Earth
gravconst_m   = G*Mm*Ms   # Mars
gravconst_c   = G*Mc*Ms   # Comet
gravconst_j   = G*Mj*Ms   # Jupiter
gravconst_sat = G*Msat*Ms # Saturn
gravconst_u   = G*Mu*Ms   # Uranus
gravconst_n   = G*Mn*Ms   # Neptune
gravconst_p   = G*Mp*Ms   # Pluto
#######################################
#
#
...
while t<25*365*daysec:
    ...
    ################ The Sun ###########
    # update quantities how is this calculated?  F = ma -> a = F/m
    xvs += -(fx_me+fx_v+fx_e+fx_m+fx_j+fx_sa+fx_u+fx_n)*dt/Ms
    yvs += -(fy_me+fy_v+fy_e+fy_m+fy_j+fy_sa+fy_u+fy_n)*dt/Ms
    zvs += -(fz_me+fz_v+fz_e+fz_m+fz_j+fz_sa+fz_u+fz_n)*dt/Ms
